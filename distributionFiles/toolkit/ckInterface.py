...
def collectMetrics(ckDir, repoPath):
    ...
    processRawCSV(readPath,writePath,repoPath)
...
def runJar(jarPath, repoPath):
    ...
    try:
        ...
        print("ck metrics output successful",output)
    except:
        print("Error: Could not run jar file.")
        return

#collectMetrics(jarPath,repoPath)    
def refineRow(row, repoPath):
...
def processRawCSV(readPath, writePath, repoPath):
    ...
    with open(readPath, 'rb') as readFrom:
        with open(writePath,'wb') as writeFrom:
            ...
            while(rows < nRows):
                ...
                if(isNext):
                    currRow = refineRow(reader.next(),repoPath)
                    rows+=1
                else:
                    currRow = nextRow
                ...
                if(rows < nRows-1):    
                    nextRow = refineRow(reader.next(),repoPath)
                    rows+=1
                    s1=str(currRow[0]) 
                    s2 = str(nextRow[0])
                    ...
                    while(currRow[0]==nextRow[0] and rows<nRows):
                        ...
                        nextRow = refineRow(next(reader),repoPath)
                        rows+=1
